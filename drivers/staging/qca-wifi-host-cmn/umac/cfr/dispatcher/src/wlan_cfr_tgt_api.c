 ...
 *
 * Relayfs is the independent all-or-drop data plane. Optional netlink
 * duplication uses fixed per-pdev storage under its own lock, so an absent or
 * slow netlink consumer cannot fragment the relay stream.
 */
