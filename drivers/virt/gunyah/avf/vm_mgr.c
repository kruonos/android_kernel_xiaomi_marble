...
static bool reset_load_vm_before_dealloc = true;
module_param_named(reset_load_vm_before_dealloc,
		   reset_load_vm_before_dealloc, bool, 0600);
MODULE_PARM_DESC(reset_load_vm_before_dealloc,
		 "Attempt VM_RESET before VM_DEALLOC_VMID after experimental LOAD-state start failures");

static uint reset_load_vm_timeout_ms = 500;
module_param_named(reset_load_vm_timeout_ms, reset_load_vm_timeout_ms, uint,
		  0600);
MODULE_PARM_DESC(reset_load_vm_timeout_ms,
		 "Timeout in milliseconds to wait for RESET after LOAD-state VM_RESET; 0 skips waiting");
...
void gunyah_vm_function_unregister(struct gunyah_vm_function *fn)
{
	 ...
	 *
	 * On this 5.10 base, delete_module() marks the module GOING before
	 * calling its exit path. module_refcount() then returns -1 for the
	 * module being unloaded, so treating every non-zero value as a leak
	 * produces a WARN_ON during normal ghfunc module unload. Only positive
	 * references indicate live users that should not be present here.
	 */
	WARN_ON(fn->mod && module_refcount(fn->mod) > 0);
...
static u32 gunyah_vm_binding_demand_page_count(struct gunyah_vm *ghvm,
					       struct gunyah_vm_gup_binding *b)
{
	u64 binding_end = b->guest_phys_addr + b->size;
	u64 dtb_start, dtb_end;
	u32 count = 0;

	if (!ghvm->dtb.bounce_active)
		return 1;

	dtb_start = ghvm->dtb.parcel_start << PAGE_SHIFT;
	dtb_end = dtb_start + (ghvm->dtb.parcel_pages << PAGE_SHIFT);

	if (binding_end <= dtb_start || dtb_end <= b->guest_phys_addr)
		return 1;

	if (b->guest_phys_addr < dtb_start)
		count++;
	if (dtb_end < binding_end)
		count++;

	return count;
}

static void gunyah_vm_binding_fill_demand_page_entries(
	struct gunyah_vm *ghvm, struct gunyah_vm_gup_binding *b,
	struct gunyah_rm_mem_entry *entries, u32 *idx)
{
	u64 binding_end = b->guest_phys_addr + b->size;
	u64 dtb_start, dtb_end;

	if (!ghvm->dtb.bounce_active) {
		entries[*idx].phys_addr = cpu_to_le64(b->guest_phys_addr);
		entries[*idx].size = cpu_to_le64(b->size);
		(*idx)++;
		return;
	}
	...
	if (b->guest_phys_addr < dtb_start) {
		entries[*idx].phys_addr = cpu_to_le64(b->guest_phys_addr);
		entries[*idx].size = cpu_to_le64(dtb_start - b->guest_phys_addr);
		(*idx)++;
	}

	if (dtb_end < binding_end) {
		entries[*idx].phys_addr = cpu_to_le64(dtb_end);
		entries[*idx].size = cpu_to_le64(binding_end - dtb_end);
		(*idx)++;
	}
}
...
int gunyah_gup_setup_demand_paging(struct gunyah_vm *ghvm)
{
	...
	mt_for_each(&ghvm->bindings, b, index, ULONG_MAX)
		if (b->share_type == VM_MEM_LEND &&
			...
			count += gunyah_vm_binding_demand_page_count(ghvm, b);
	...
	mt_for_each(&ghvm->bindings, b, index, ULONG_MAX) {
		if (b->share_type != VM_MEM_LEND ||
			(b->guest_phys_addr == ghvm->fw.config.guest_phys_addr))
			continue;
		gunyah_vm_binding_fill_demand_page_entries(ghvm, b, entries, &i);
		if (i == count)
			break;
	}
...
static int gunyah_vm_start(struct gunyah_vm *ghvm)
{
	...
	ret = gunyah_gup_share_dtb_bounce_parcel(ghvm);
	if (ret == -EOPNOTSUPP) {
			       ...
			       u64 nr)
{
	...
	if (parcel->mem_handle != GUNYAH_MEM_HANDLE_INVAL) {
		...
		ret = gunyah_vm_parcel_to_hlos(parcel);
		if (ret) {
			dev_err(ghvm->parent,
				"Failed to assign parcel back to HLOS: %d\n", ret);
			return ret;
		}
...
static void _gunyah_vm_put(struct kref *kref)
{
	...
	if (ghvm->dtb.bounce_active) {
		ret = gunyah_gup_reclaim_dtb_bounce_parcel(ghvm);
		if (ret)
			dev_err(ghvm->parent,
				"Failed to reclaim DTB bounce parcel: %d\n", ret);
	} else if (ghvm->vm_status == GUNYAH_RM_VM_STATUS_LOAD ||
		   ghvm->vm_status == GUNYAH_RM_VM_STATUS_READY ||
		   ghvm->vm_status == GUNYAH_RM_VM_STATUS_INIT_FAILED) {
	...
	if (reset_load_vm_before_dealloc &&
	    ghvm->vm_status == GUNYAH_RM_VM_STATUS_LOAD) {
		ret = gunyah_rm_vm_reset(ghvm->rm, ghvm->vmid);
		if (ret) {
			dev_warn(ghvm->parent,
				 "Failed to reset LOAD-state vm before dealloc: %d\n",
				 ret);
		} else if (reset_load_vm_timeout_ms) {
			unsigned long timeout = msecs_to_jiffies(reset_load_vm_timeout_ms);

			if (!timeout)
				timeout = 1;
			if (!wait_event_timeout(ghvm->vm_status_wait,
						ghvm->vm_status == GUNYAH_RM_VM_STATUS_RESET,
						timeout))
				dev_warn(ghvm->parent,
					 "Timed out waiting for LOAD-state vm reset before dealloc\n");
		}
	}
