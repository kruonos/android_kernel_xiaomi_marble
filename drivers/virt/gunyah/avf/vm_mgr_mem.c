...
static bool legacy_hyp_assign_mem;
module_param_named(legacy_hyp_assign_mem, legacy_hyp_assign_mem, bool, 0600);
MODULE_PARM_DESC(legacy_hyp_assign_mem,
		 "Use legacy Qualcomm hyp_assign_table around experimental AVF Gunyah memory parcels; disabled by default");

static bool dtb_bounce_contig = true;
module_param_named(dtb_bounce_contig, dtb_bounce_contig, bool, 0600);
MODULE_PARM_DESC(dtb_bounce_contig,
		 "Use a physically contiguous DTB bounce parcel for the experimental AVF image handoff");

static bool dtb_bounce_hyp_assign;
module_param_named(dtb_bounce_hyp_assign, dtb_bounce_hyp_assign, bool, 0600);
MODULE_PARM_DESC(dtb_bounce_hyp_assign,
		 "Apply legacy hyp_assign_phys to the contiguous DTB bounce parcel before MEM_LEND; disabled by default");

static int gunyah_acl_perms_to_hyp_perms(u8 perms)
{
	int hyp_perms = 0;

	if (perms & GUNYAH_RM_ACL_R)
		hyp_perms |= PERM_READ;
	if (perms & GUNYAH_RM_ACL_W)
		hyp_perms |= PERM_WRITE;
	if (perms & GUNYAH_RM_ACL_X)
		hyp_perms |= PERM_EXEC;

	return hyp_perms;
}

static u8 gunyah_binding_flags_to_acl_perms(u32 flags)
{
	u8 perms = 0;

	if (flags & GUNYAH_MEM_ALLOW_READ)
		perms |= GUNYAH_RM_ACL_R;
	if (flags & GUNYAH_MEM_ALLOW_WRITE)
		perms |= GUNYAH_RM_ACL_W;
	if (flags & GUNYAH_MEM_ALLOW_EXEC)
		perms |= GUNYAH_RM_ACL_X;

	return perms;
}

static int gunyah_dtb_bounce_assign_phys(struct gunyah_vm *ghvm,
					 bool to_guest)
{
	phys_addr_t phys;
	u32 src_vmlist[1], dst_vmlist[1];
	int dst_perms[1];
	int ret;

	if (!ghvm->dtb.bounce_buf || !ghvm->dtb.bounce_size)
		return -EINVAL;

	phys = page_to_phys(virt_to_page(ghvm->dtb.bounce_buf));

	if (to_guest) {
		src_vmlist[0] = VMID_HLOS;
		dst_vmlist[0] = ghvm->vmid;
		dst_perms[0] = gunyah_acl_perms_to_hyp_perms(
			ghvm->dtb.parcel.acl_entries[0].perms);
		if (!dst_perms[0])
			return -EINVAL;
	} else {
		src_vmlist[0] = ghvm->vmid;
		dst_vmlist[0] = VMID_HLOS;
		dst_perms[0] = PERM_READ | PERM_WRITE | PERM_EXEC;
	}

	ret = hyp_assign_phys(phys, ghvm->dtb.bounce_size, src_vmlist, 1,
			     dst_vmlist, dst_perms, 1);
	if (ret)
		pr_err("legacy hyp_assign_phys %s DTB bounce parcel failed: %d\n",
		       to_guest ? "to guest" : "back to HLOS", ret);

	return ret;
}

int gunyah_gup_share_dtb_bounce_parcel(struct gunyah_vm *ghvm)
{
	struct gunyah_vm_gup_binding *b;
	struct gunyah_rm_mem_parcel *parcel = &ghvm->dtb.parcel;
	struct page **pages = NULL;
	void *bounce_buf = NULL;
	u64 parcel_start_gpa, parcel_size;
	u64 parcel_offset, offset;
	unsigned long i;
	unsigned long parcel_pages;
	unsigned int gup_flags = 0;
	int pinned, ret;
	u8 perms;

	if (!dtb_bounce_contig)
		return -EOPNOTSUPP;

	ghvm->dtb.bounce_active = false;
	ghvm->dtb.bounce_hyp_assigned = false;
	ghvm->dtb.bounce_buf = NULL;
	ghvm->dtb.bounce_size = 0;

	parcel_start_gpa = round_down(ghvm->dtb.config.guest_phys_addr,
				      (u64)PAGE_SIZE);
	parcel_offset = ghvm->dtb.config.guest_phys_addr - parcel_start_gpa;
	if (ghvm->dtb.config.size > U64_MAX - parcel_offset)
		return -EOVERFLOW;

	parcel_size = parcel_offset + ghvm->dtb.config.size;
	if (parcel_size > U64_MAX - (PAGE_SIZE - 1))
		return -EOVERFLOW;
	parcel_size = PAGE_ALIGN(parcel_size);
	if (!parcel_size)
		return -EINVAL;

	parcel_pages = parcel_size >> PAGE_SHIFT;

	parcel->mem_handle = GUNYAH_MEM_HANDLE_INVAL;
	ghvm->dtb.parcel_start = parcel_start_gpa >> PAGE_SHIFT;
	ghvm->dtb.parcel_pages = parcel_pages;

	down_read(&ghvm->bindings_lock);
	b = mtree_load(&ghvm->bindings, ghvm->dtb.parcel_start);
	if (!b) {
		ret = -ENOENT;
		goto unlock;
	}

	offset = parcel_start_gpa - b->guest_phys_addr;
	if (offset > b->size || parcel_size > b->size - offset) {
		ret = -ERANGE;
		goto unlock;
	}

	pages = kcalloc(parcel_pages, sizeof(*pages), GFP_KERNEL_ACCOUNT);
	if (!pages) {
		ret = -ENOMEM;
		goto unlock;
	}
	...
	pinned = pin_user_pages_fast(b->userspace_addr + offset, parcel_pages,
				     gup_flags, pages);
	if (pinned < 0) {
		ret = pinned;
		goto free_pages;
	} else if (pinned != parcel_pages) {
		ret = -EFAULT;
		goto unpin_pages;
	}

	bounce_buf = alloc_pages_exact(parcel_size, GFP_KERNEL | __GFP_ZERO);
	if (!bounce_buf) {
		ret = -ENOMEM;
		goto unpin_pages;
	}

	for (i = 0; i < parcel_pages; i++) {
		void *src = kmap(pages[i]);

		memcpy((char *)bounce_buf + (i * PAGE_SIZE), src, PAGE_SIZE);
		kunmap(pages[i]);
	}

	parcel->mem_type = GUNYAH_RM_MEM_TYPE_NORMAL;
	...
	perms = gunyah_binding_flags_to_acl_perms(b->flags);
	if (!perms) {
		ret = -EINVAL;
		goto free_acl;
	}
	...
	parcel->mem_entries[0].phys_addr =
		cpu_to_le64(page_to_phys(virt_to_page(bounce_buf)));
	parcel->mem_entries[0].size = cpu_to_le64(parcel_size);

	ghvm->dtb.bounce_buf = bounce_buf;
	ghvm->dtb.bounce_size = parcel_size;
	ghvm->dtb.bounce_active = true;

	if (dtb_bounce_hyp_assign) {
		ret = gunyah_dtb_bounce_assign_phys(ghvm, true);
		if (ret)
			goto free_mem;
		ghvm->dtb.bounce_hyp_assigned = true;
	}
	...
	dev_info(ghvm->parent,
		 "Using contiguous DTB bounce parcel: label=%u pages=%lu size=%#llx\n",
		 parcel->label, parcel_pages, parcel_size);
	ret = 0;
	goto unpin_pages;

assign_back:
	if (ghvm->dtb.bounce_hyp_assigned) {
		int assign_ret = gunyah_dtb_bounce_assign_phys(ghvm, false);

		if (!assign_ret) {
			ghvm->dtb.bounce_hyp_assigned = false;
		} else {
			pr_err("failed to reassign rejected DTB bounce parcel back to HLOS: %d\n",
			       assign_ret);
			ret = assign_ret;
			goto unpin_pages;
		}
...
free_acl:
	...
	parcel->n_acl_entries = 0;
free_bounce:
	free_pages_exact(bounce_buf, parcel_size);
	bounce_buf = NULL;
	ghvm->dtb.bounce_buf = NULL;
	ghvm->dtb.bounce_size = 0;
	ghvm->dtb.bounce_active = false;
...
int gunyah_gup_reclaim_dtb_bounce_parcel(struct gunyah_vm *ghvm)
{
	struct gunyah_rm_mem_parcel *parcel = &ghvm->dtb.parcel;
	int ret;

	if (!ghvm->dtb.bounce_active)
		return 0;
	...
	if (parcel->mem_handle != GUNYAH_MEM_HANDLE_INVAL) {
		...
		if (ret) {
			dev_err(ghvm->parent,
				"Failed to reclaim DTB bounce parcel: %d\n", ret);
			return ret;
		}
	...
	if (ghvm->dtb.bounce_hyp_assigned) {
		ret = gunyah_dtb_bounce_assign_phys(ghvm, false);
		if (ret)
			return ret;
		ghvm->dtb.bounce_hyp_assigned = false;
	}
	...
	free_pages_exact(ghvm->dtb.bounce_buf, ghvm->dtb.bounce_size);
	ghvm->dtb.bounce_buf = NULL;
	ghvm->dtb.bounce_size = 0;
	ghvm->dtb.bounce_active = false;
...
static int gunyah_parcel_to_sg_table(struct gunyah_rm_mem_parcel *parcel,
				     struct sg_table *sgt)
{
	...
	if (!parcel->n_mem_entries || !parcel->mem_entries)
		return -EINVAL;

	ret = sg_alloc_table(sgt, parcel->n_mem_entries, GFP_KERNEL);
	if (ret)
		return ret;

	for_each_sg(sgt->sgl, sg, sgt->orig_nents, i) {
		phys_addr_t phys = le64_to_cpu(parcel->mem_entries[i].phys_addr);
		u64 size = le64_to_cpu(parcel->mem_entries[i].size);
		...
		sg_set_page(sg, phys_to_page(phys), size, 0);
	}
...
static int gunyah_vm_parcel_to_acl(struct gunyah_rm_mem_parcel *parcel)
{
	struct sg_table sgt;
	...
	if (!legacy_hyp_assign_mem)
		return 0;

	if (!parcel->n_acl_entries || !parcel->acl_entries)
		return -EINVAL;

	dst_vmids = kcalloc(parcel->n_acl_entries, sizeof(*dst_vmids),
			    GFP_KERNEL);
	if (!dst_vmids)
		return -ENOMEM;

	dst_perms = kcalloc(parcel->n_acl_entries, sizeof(*dst_perms),
			    GFP_KERNEL);
	if (!dst_perms) {
		ret = -ENOMEM;
		goto free_dst_vmids;
	...
	for (i = 0; i < parcel->n_acl_entries; i++) {
		dst_vmids[i] = le16_to_cpu(parcel->acl_entries[i].vmid);
		dst_perms[i] = gunyah_acl_perms_to_hyp_perms(
			parcel->acl_entries[i].perms);
		if (!dst_perms[i]) {
			ret = -EINVAL;
			goto free_dst_perms;
		}
	...
	ret = gunyah_parcel_to_sg_table(parcel, &sgt);
	if (ret)
		goto free_dst_perms;

	ret = hyp_assign_table(&sgt, &src_vmid, 1, dst_vmids, dst_perms,
			       parcel->n_acl_entries);
	sg_free_table(&sgt);
	if (ret)
		pr_err("legacy hyp_assign_table to ACL failed: %d\n", ret);
...
int gunyah_vm_parcel_to_hlos(struct gunyah_rm_mem_parcel *parcel)
{
	struct sg_table sgt;
	u32 *src_vmids;
	...
	src_vmids = kcalloc(parcel->n_acl_entries, sizeof(*src_vmids),
			    GFP_KERNEL);
	if (!src_vmids)
		return -ENOMEM;

	for (i = 0; i < parcel->n_acl_entries; i++)
		src_vmids[i] = le16_to_cpu(parcel->acl_entries[i].vmid);

	ret = gunyah_parcel_to_sg_table(parcel, &sgt);
	if (ret)
		goto free_src_vmids;

	ret = hyp_assign_table(&sgt, src_vmids, parcel->n_acl_entries,
			       &dst_vmid, &dst_perms, 1);
	sg_free_table(&sgt);
	if (ret)
		pr_err("legacy hyp_assign_table back to HLOS failed: %d\n", ret);
...
static void gunyah_gup_release_invalid_parcel(struct gunyah_rm_mem_parcel *parcel)
{
	size_t i;

	for (i = 0; i < parcel->n_mem_entries; i++) {
		struct gunyah_rm_mem_entry *entry = &parcel->mem_entries[i];
		struct folio *folio;
...
int gunyah_gup_demand_page(struct gunyah_vm *ghvm, u64 gpa, bool write)
{
	...
	if (ghvm->dtb.bounce_active &&
	    gpa >= (ghvm->dtb.parcel_start << PAGE_SHIFT) &&
	    gpa < ((ghvm->dtb.parcel_start + ghvm->dtb.parcel_pages)
		   << PAGE_SHIFT)) {
		pr_warn_ratelimited(
			"Refusing to demand-page DTB bounce GPA %#llx; RM image parcel should already cover it\n",
			gpa);
		ret = -ENOENT;
		goto unlock;
	}
			     ...
			     u64 *gfn, u64 *nr)
{
	...
	*nr = folio_nr_pages(folio);
	...
	ret = gunyah_vm_parcel_to_acl(parcel);
	if (ret)
		goto release_invalid_parcel;

	ret = gunyah_rm_mem_share(ghvm->rm, parcel);
	if (ret) {
		int assign_ret = gunyah_vm_parcel_to_hlos(parcel);

		if (assign_ret) {
			pr_err("failed to assign rejected parcel back to HLOS: %d; leaving pages pinned\n",
			       assign_ret);
			goto free_pages;
		}
		goto release_invalid_parcel;
...
release_invalid_parcel:
	gunyah_gup_release_invalid_parcel(parcel);
	goto free_pages;
free_acl:
	kfree(parcel->acl_entries);
	parcel->acl_entries = NULL;
