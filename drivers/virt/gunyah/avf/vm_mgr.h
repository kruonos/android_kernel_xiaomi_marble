...
{
	struct address_space *mapping = page_mapping(folio_page(folio, 0));
...
static inline bool gunyah_folio_lend_safe(struct folio *folio)
{
	...
	/* Bouquet 5.10 keeps this AVF path module-local during bring-up; do not
	 * call non-exported direct-map helpers from the module until the real memory
	 * isolation strategy is validated.
	 */
	/**
...
static inline void gunyah_folio_host_reclaim(struct folio *folio)
{
	...
	/* See gunyah_folio_lend_safe(): no direct-map remap was performed in this
	 * compile-only backport scaffold.
	 */
}
