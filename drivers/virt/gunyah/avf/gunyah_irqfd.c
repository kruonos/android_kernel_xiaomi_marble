			...
			void *key)
{
	...
	if (flags & EPOLLIN) {
		struct gunyah_resource *ghrsc = READ_ONCE(irqfd->ghrsc);

		if (ghrsc) {
			ret = gunyah_hypercall_bell_send(ghrsc->capid, 1, NULL);
			if (ret)
				pr_err_ratelimited(
				  ...
				  struct gunyah_resource *ghrsc)
{
	...
	WRITE_ONCE(irqfd->ghrsc, ghrsc);
	if (irqfd->level) {
		/* Configure the bell to trigger when bit 0 is asserted and for
		 * bell to automatically clear bit 0 once received by the VM.
		 */
				    ...
				    struct gunyah_resource *ghrsc)
{
	...
	if (READ_ONCE(irqfd->ghrsc) == ghrsc)
		WRITE_ONCE(irqfd->ghrsc, NULL);
}
