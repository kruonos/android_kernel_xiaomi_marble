...
static bool coalesce_mem_entries = true;
module_param_named(coalesce_mem_entries, coalesce_mem_entries, bool, 0600);
MODULE_PARM_DESC(coalesce_mem_entries,
		 "Coalesce adjacent AVF memparcel entries before sending RM MEM_LEND/MEM_SHARE requests");

static uint max_mem_request_size = SZ_8K;
module_param_named(max_mem_request_size, max_mem_request_size, uint, 0600);
MODULE_PARM_DESC(max_mem_request_size,
		 "Maximum encoded AVF RM memory request payload size; 0 disables the guard");
...
static bool gunyah_rm_mem_request_size(size_t n_acl_entries,
				       size_t n_mem_entries, size_t *msg_size)
{
	struct gunyah_rm_mem_share_req_acl_section *acl;
	struct gunyah_rm_mem_share_req_mem_section *mem;
	size_t acl_size, mem_size, size;

	if (n_acl_entries > U16_MAX || n_mem_entries > U16_MAX)
		return false;

	acl_size = struct_size(acl, entries, n_acl_entries);
	mem_size = struct_size(mem, entries, n_mem_entries);

	if (check_add_overflow(sizeof(struct gunyah_rm_mem_share_req_header),
			       acl_size, &size) ||
	    check_add_overflow(size, mem_size, &size) ||
	    check_add_overflow(size, sizeof(u32), &size))
		return false;
...
static int gunyah_rm_coalesce_mem_entries(struct gunyah_rm_mem_entry *entries,
					  size_t n_entries,
					  struct gunyah_rm_mem_entry **out_entries,
					  size_t *out_n_entries)
{
	struct gunyah_rm_mem_entry *coalesced;
	size_t i, n = 0;

	*out_entries = entries;
	*out_n_entries = n_entries;

	if (!coalesce_mem_entries || n_entries < 2)
		return 0;

	coalesced = kcalloc(n_entries, sizeof(*coalesced), GFP_KERNEL);
	if (!coalesced)
		return -ENOMEM;

	for (i = 0; i < n_entries; i++) {
		u64 phys = le64_to_cpu(entries[i].phys_addr);
		u64 size = le64_to_cpu(entries[i].size);
		...
		if (n) {
			u64 prev_phys = le64_to_cpu(coalesced[n - 1].phys_addr);
			u64 prev_size = le64_to_cpu(coalesced[n - 1].size);
			u64 prev_end;

			if (!check_add_overflow(prev_phys, prev_size, &prev_end) &&
			    prev_end == phys &&
			    !check_add_overflow(prev_size, size, &prev_size)) {
				coalesced[n - 1].size = cpu_to_le64(prev_size);
				continue;
			}
		}

		coalesced[n++] = entries[i];
	}
	...
	pr_info("gunyah_rsc_mgr: coalesced memparcel entries %zu -> %zu\n",
		n_entries, n);
	*out_entries = coalesced;
	*out_n_entries = n;
	return 0;
}
				...
				size_t n_entries)
{
	...
	struct gunyah_rm_mem_share_req_mem_section *mem;
	size_t max_entries = GUNYAH_RM_MAX_MEM_ENTRIES;
	int ret = 0;
	size_t n;

	if (max_mem_request_size) {
		size_t overhead = sizeof(*req) + sizeof(*mem);

		if (max_mem_request_size <= overhead)
			return -E2BIG;

		max_entries = min_t(size_t, max_entries,
				(max_mem_request_size - overhead) / sizeof(*entries));
		if (!max_entries)
			return -E2BIG;
	}
...
int gunyah_rm_mem_share(struct gunyah_rm *rm, struct gunyah_rm_mem_parcel *p)
{
	...
	size_t msg_size, initial_mem_entries, resp_size;
	struct gunyah_rm_mem_share_req_acl_section *acl;
	struct gunyah_rm_mem_share_req_mem_section *mem;
	struct gunyah_rm_mem_share_req_header *req_header;
	struct gunyah_rm_mem_entry *send_entries;
	size_t send_n_mem_entries;
	...
	ret = gunyah_rm_coalesce_mem_entries(p->mem_entries, p->n_mem_entries,
					       &send_entries, &send_n_mem_entries);
	if (ret)
		return ret;

	initial_mem_entries = send_n_mem_entries;
	 ...
	 */
	if (!gunyah_rm_mem_request_size(p->n_acl_entries, initial_mem_entries,
					  &msg_size)) {
		ret = -EOVERFLOW;
		goto free_send_entries;
	}

	if (max_mem_request_size && msg_size > max_mem_request_size) {
		pr_warn("gunyah_rsc_mgr: refusing oversized %s request: encoded=%zu limit=%u acl=%zu mem_entries=%zu total_entries=%zu\n",
			message_id == GUNYAH_RM_RPC_MEM_LEND ? "MEM_LEND" : "MEM_SHARE",
			msg_size, max_mem_request_size, p->n_acl_entries,
			initial_mem_entries, send_n_mem_entries);
		ret = -E2BIG;
		goto free_send_entries;
	}
	...
	memcpy(mem->entries, send_entries,
	...
	*attr_section = 0;
	...
	if (need_append) {
		ret = gunyah_rm_mem_append(
			rm, p->mem_handle, &send_entries[initial_mem_entries],
			send_n_mem_entries - initial_mem_entries);
		if (ret) {
			int reclaim_ret = gunyah_rm_mem_reclaim(rm, p);
